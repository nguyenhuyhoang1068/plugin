...
//        wp_enqueue_script('openpos.kitchen.nosleep', OPENPOS_URL.'/assets/js/NoSleep.min.js','',$info['Version']);
    ...
    function update_order(){
        ...
        try{
            ...
            if((!$is_exchange && !$is_refund) || !$order_id)
            {
                ...
                update_post_meta($order_id,'_op_order',$order_post_data);
            }
    ...
    function add_order($is_clear = false){
        ...
        try{
            ...
            $total_paid = isset($order_parse_data['total_paid']) ? $order_parse_data['total_paid'] : 0;
                ...
                if($order_number > 0)
                {
                    ...
                    if(!$order_post)
                    {
                        ...
                        if(!$pos_use_offline_order_number && $order_source == 'direct')
                        {
                            ...
                            $_order_post = $op_woo_order->reGenerateDraftOrder($order_id,$new_order_number,$order_number_format);
                    ...
                    if($order_post->post_status == 'draft' || $order_post->post_status == 'auto-draft')
                    {
        ...
        try{
            ...
            if((int)$order_number > 0)
            {
                ...
                if(count($orders) > 0)
                {
                    ...
                    do_action( 'op_woocommerce_cancelled_order', $_order->ID, $status );
