	...
	// TRG-2 END 
        ...
        if($pos_enable_weight_barcode == 'yes' )
        {
            $pos_weight_barcode_format = array(
                ...
                'desc'    => __('Work with barcode scanner device only. I : item code , P : price , W : weight  , Q : quantity . Example: "DDIIIIIDPPPPC" -   "2081981002076" With "20": string to detect barcode generate by scale, "81981" : product barcode , "0207" : price = 2.07$  ', 'openpos' ),
                'type'    => 'text',
...
// TRG -2. END 
    ...
    public function qrcode_table(){
        ...
        try{
            ...
            $login_url = '';
            $base_url = apply_filters('op_get_base_url',OPENPOS_URL);
            if($key && $register)
            {
                $login_url = $base_url.'/customer/index.php?key='.esc_attr($key);
    ...
    public function geneate_qrcode_table(){
        ...
        try{
            ...
            if($key && $register)
            {
                $login_url = OPENPOS_URL.'/customer/index.php?key='.esc_attr($key);
                $image_url = 'https://chart.googleapis.com/chart?chs=250x250&cht=qr&choe=UTF-8&chl='.urlencode($login_url);
            }
            $result['data']['table'] = $table;
